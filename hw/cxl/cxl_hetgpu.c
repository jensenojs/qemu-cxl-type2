...
typedef int (*cuMemcpy2DAsync_fn)(const CudaMemcpy2D *, void *);
...
typedef int (*cuGraphKernelNodeSetParams_fn)(void *,
                                             const CudaKernelNodeParams *);
typedef int (*cuGraphMemcpyNodeGetParams_fn)(void *, CudaMemcpy3DParams *);
...
static struct {
    ...
    cuMemcpy2DAsync_fn cuMemcpy2DAsync;
    ...
    cuGraphMemcpyNodeGetParams_fn cuGraphMemcpyNodeGetParams;
                                        ...
                                        bool require_kimi_hooks)
{
    ...
    if (!g_cuda_lib_initialized) {
        ...
        if (g_cuda_lib_handle) {
            ...
            g_cuda_funcs.cuMemcpy2DAsync =
                dlsym(g_cuda_lib_handle, "cuMemcpy2DAsync_v2");
            ...
            g_cuda_funcs.cuGraphMemcpyNodeGetParams =
                dlsym(g_cuda_lib_handle, "cuGraphMemcpyNodeGetParams");
                                          ...
                                          const char *reason)
{
    ...
    if (state && state->param_info_cache) {
        entries = g_hash_table_size(state->param_info_cache);
...
HetGPUError hetgpu_memcpy2d_dtod_async(HetGPUState *state,
                                       ...
                                       HetGPUStream stream)
{
    if (!state || !state->initialized || !g_cuda_funcs.cuMemcpy2DAsync ||
        state->backend == HETGPU_BACKEND_SIMULATION) {
        return HETGPU_ERROR_INVALID_VALUE;
    }
    ...
    int err = HETGPU_CUDA_CALL(cuMemcpy2DAsync, &copy, stream);
    cuda_unlock(state);
    if (err != 0) {
        const char *err_name = "UNKNOWN";
        if (g_cuda_funcs.cuGetErrorName) {
        ...
        qemu_log("CXL hetGPU: [dev%d] cuMemcpy2DAsync DtoD failed: %s (%d)\n",
                 state->device_index, err_name, err);
        return HETGPU_ERROR_INVALID_VALUE;
    }
    return HETGPU_SUCCESS;
...
{
    if (!state || !state->initialized || !function || !layout) {
        return HETGPU_ERROR_INVALID_VALUE;
    }

    HetGPUParamLayout *cached = state->param_info_cache ?
    ...
    if (g_cuda_funcs.cuFuncGetParamInfo &&
        ...
        g_hash_table_insert(state->param_info_cache, function, candidate);
        *layout = candidate;
        return HETGPU_SUCCESS;
    }
                                             ...
                                             CudaMemcpy3DParams *params)
{
    ...
    int result = HETGPU_CUDA_CALL(cuGraphMemcpyNodeGetParams, graph_node, params);
    cuda_unlock(state);
    return result;
}
